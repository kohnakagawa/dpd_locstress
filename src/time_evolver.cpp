...
#pragma omp parallel for
  for(int i=0; i < Parameter::SYS_SIZE; i++){
    ...
    pr[i].x = temp_r.x - floor(temp_r.x * p_param->iL.x) * p_param->L.x;

#ifdef Y_REFLECT_BOUND
    ReflectBound(temp_r.y, pr[i].y, pv_bef[i], force_bef[i], p_param->L.y);
#else
    pr[i].y = temp_r.y - floor(temp_r.y * p_param->iL.y) * p_param->L.y;
#endif    
    
    pr[i].z = temp_r.z - floor(temp_r.z * p_param->iL.z) * p_param->L.z;
