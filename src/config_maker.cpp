...
class ConfigMaker {
  ...
  double CalcAmpRatio(const Parameter& param) {
    return static_cast<double>(param.hN + param.bN) / static_cast<double>(param.sys_size);
  }
  ...
  void MakeRandom(prng_gen& prng,
    ...
    while (added_amp_num < param.ampN) {
  ...
  void MakeRandom(prng_gen& prng,
                  ...
                  int& added_num,
                  const Parameter& param) {
    const double3 up(0.0, 0.0, 0.0);
    const double3 dw = param.L;
    MakeRandom<prng_gen>(prng, uniform, added_num, up, dw, param);
  }
  ...
  void GenConfiguration(prng_gen& prng, const Parameter& param) {
    ...
    if (mode == "Flat") {
      ...
      std::cout << "We make oil/water interface.\n";
      const int axis = 1;
      const double3 dw(0.0, 0.0, 0.0);
      double3 up = param.L;
      up[axis] = param.L[axis] * CalcAmpRatio(param);
      MakeRandom(prng, uniform, added_num, up, dw, param);
      FillWaterPtclsRect(prng, uniform, added_num, axis, param);
    } else {
