...
B_sorter::B_sorter(const Parameter& param){
  interact_list   = new InteractList [param.all_grid];
	...
	for(int jz=-1; jz<2; jz++){
	  for(int jy=-1; jy<2; jy++){
	    for(int jx=-1; jx<2; jx++){
	      ...
	      interact_list[tar_grid].l[inr_grid] = GenHash(q,param);
	      inr_grid++;
...
void B_sorter::BucketSort(dpdsystem& sDPD,ChemInfo& cheminfo,const Parameter& param,const CellList& celllist){
  //copy & gather
...
void B_sorter::MkPrtclIdx(const double3* pr,const Parameter& param,CellList& celllist){
    //init bucket
...
#pragma omp parallel for
  for(int tar_grid=0; tar_grid<param.all_grid; tar_grid++){
    ...
    for(int i=0; i<13; i++){
      const int intr_grid = interact_list[tar_grid].l[i];
...
void B_sorter::CheckSorted(const dpdsystem& sDPD,const Parameter& param,const CellList& celllist) const {
  /*for(int i=0; i<param.all_grid; i++){
    for(int j=0; j<13; j++){
    std::cout << interact_list[i].l[j] << std::endl;
    }
    }*/
...
void B_sorter::CheckNearList(const dpdsystem& sDPD,const Parameter& param) const {
  ...
  for(int i=0; i<Parameter::SYS_SIZE-1; i++){
    for(int j=i+1; j<Parameter::SYS_SIZE; j++){
      ...
      dr.x -= param.L.x * (int) (dr.x * param.ihL.x);
      dr.y -= param.L.y * (int) (dr.y * param.ihL.y);
      dr.z -= param.L.z * (int) (dr.z * param.ihL.z);
