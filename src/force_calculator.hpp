...
class F_calculator{
  ...
  int grid_numbtw, numb_band, p_num_band[3];
  ...
  INLINE void StoreBondForce(const double3& __restrict dr,const double& __restrict inv_dr,
			     double3& d_virial, double& lap_conf,
			     double3* __restrict F,const Parameter& param)
  {
  ...
  void RegistNearIdx(const int& itr_id,const int& tar_id,
		     const float& dist,
		     ChemInfo& cheminfo,const Parameter& param);

  void CountNearWater(const int& itr_id,     const int& tar_id,		      
		      const par_prop& i_prp, const par_prop& t_prp,
		      float dist,
		      ChemInfo& cheminfo,    const Parameter& param);
  ...
  void Collision_pow_one(double3& vi, double3& vj, const double3& dr, const double cf_g,
			 const double nrml, const double inv_dr, const Parameter& param)
  {
  ...
  void Collision_pow_half(double3& vi, double3& vj, const double3& dr, const double cf_g,
			  const double nrml, const double inv_dr, const double dr_norm, 
			  const Parameter& param)
  {
...
public:
  ...
  double DumpConfigT(const double3* F){
    const double lap_pot = std::accumulate(buf_lap_pot.begin(), buf_lap_pot.end(), 0.0);
    double numer = 0.0;
    for(int i=0; i<Parameter::SYS_SIZE; i++) numer += F[i] * F[i];
    return numer / lap_pot;
  }

  double3 DumpVirial(const Parameter& param){
    double3 ret(0.0);
    for(size_t i=0; i<buf_vir.size(); i++) ret += buf_vir[i];
    return ret;
  }
};
