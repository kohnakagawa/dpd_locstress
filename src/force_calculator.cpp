...
#pragma omp parallel 
  {
    CalcForceHalf(pr, prop, force, clist, param, cheminfo, 0);
#pragma omp barrier
    CalcForceHalf(pr, prop, force, clist, param, cheminfo, 1);
  }
...
//ASSUME: tailN <= headN
void F_calculator::RegistNearIdx(const int&	itr_id,const int& tar_id,
				 const float&	dist,
...
void F_calculator::CountNearWater(const int& itr_id,     const int& tar_id,		      
				 ...
				 const int                  call_num)
{
  ...
  for(int i=begi; i<endi; i++){
	...
	RegistNearIdx(pi,pj,dist,cheminfo,param);
#ifdef INVERSE_CHEM
	CountNearWater(pi,pj,prp_pi,prp_pj,dist,cheminfo,param);
#endif
#endif
